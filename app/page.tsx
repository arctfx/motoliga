...
function TelemetryPanel() {
  return (
    <div className="mb-6 border" style={{ borderColor: "var(--ml-border)", background: "var(--ml-surface)" }}>
      <div className="px-4 pt-4 pb-3 border-b" style={{ borderColor: "var(--ml-border)" }}>
        <span className="font-mono text-[11px] tracking-[0.16em] uppercase text-ml-text-faint">
          Race Summary
        </span>
      </div>
      <div>
        <div className="flex items-center gap-3 px-4 py-3 font-mono text-xs">
          <span className="text-ml-text-ghost w-20 shrink-0">START</span>
          <span className="text-ml-text">Started in P4</span>
        </div>
        <div className="flex items-center gap-3 px-4 py-3 font-mono text-xs" style={{ borderTop: "1px solid var(--ml-border)" }}>
          <span className="text-ml-text-ghost w-20 shrink-0">LAP 1</span>
          <span className="text-ml-text">Overtake — moved to P3</span>
        </div>
        <div className="flex items-center gap-3 px-4 py-3 font-mono text-xs" style={{ borderTop: "1px solid var(--ml-border)" }}>
          <span className="text-ml-text-ghost w-20 shrink-0">LAP 8</span>
          <span style={{ color: "#ff9900" }} className="font-mono text-xs">Red flag — race suspended</span>
        </div>
        <div className="flex items-center gap-3 px-4 py-3 font-mono text-xs" style={{ borderTop: "1px solid var(--ml-border)" }}>
...
const BufferedLiveStream = React.memo(function BufferedLiveStream({ youtubeId }: { youtubeId: string }) {
  ...
  // How many seconds into the video we should be right now
    ...
    loadYouTubeApi().then(() => {
      ...
      playerRef.current = new (window as any).YT.Player(containerRef.current, {
        ...
        playerVars: {
          ...
          modestbranding: 1,
          rel: 0,
          start: getLivePosition(),
        },
        events: {
          onReady: () => {
            // Seek again on ready in case start param drifted
            playerRef.current?.seekTo?.(getLivePosition(), true);
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ background: "#000" }}>
      ...
      <div className="flex items-center justify-between px-3 py-2" style={{ background: "var(--ml-surface)" }}>
        <span
          className="relative overflow-hidden flex items-center gap-1.5 font-mono text-[11px] font-bold tracking-[0.1em] uppercase px-1"
